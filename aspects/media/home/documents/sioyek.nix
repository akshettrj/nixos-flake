...
}:
{
    config =
        ...
        in
        lib.mkIf (biryani_media.enable && biryani_docs.sioyek.enable) {
            programs.sioyek = {
                enable = true;
                ...
                bindings = {
                    goto_beginning = [
                        "gg"
                        "<c-<home>>"
                    ]; # Goto the beginning of document. If prefixed with a number, it will go to that page. For example 150gg goes to page 150.
                    goto_end = [
                        "G"
                        "<end>"
